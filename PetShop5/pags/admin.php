        ...
        <table class="table_users table-bordere-dark"> <!--Tabela HTML-->
            ...
            <?php
                while($row_user_a = $result_users_a->fetch(PDO::FETCH_ASSOC)){ // outro método extração
                    ...
                    echo "<tr><td class='table_content'>$id </td><td class='table_content'>$nome </td><td class='table_content'>$email </td><td class='table_content'>$senha </td><td class='table_content'>$telefone </td><td class='table_content'>$complemento </td><td class='table_content'>$nivel_acesso_id </td><td class='table_content'>$cep_id </td><td class='table_content'>".date('d/m/y H:i:s', strtotime($created))."</td><td class='table_content'>".date('d/m/y H:i:s', strtotime($modified))."</td>";
                    // Desde o while até o fim do echo acima, o código busca dados da tabela usuário do banco de dados, extrai eles e separa-os em colunas dentro de linhas. 
                    // Cada $ (variável) reprsenta um campo na tabela usuário, e irá se repetir até listar todos os itens da tabela. 
                    //O strtotime converte valores string (texto) em time (tempo) nas variáveis $created e $modified
                    if(!empty($modified)){ // Caso o campo modified estiver vazio ele realikza o que estiver abaixo
                        /*echo date('d/m/y H:i:s', strtotime($modified))."</td>"; */ // If 1
                        echo "Sem modificação";
                    }"<td><a class'btn btn-sm btn primary' href='edit.php?id=$row_user_a[id]</tr>"; // Cria botões de exclusão e modificação (ainda não estão prontos!!)
                    echo "<td class='table_content'><a href=deleta_serv.php?id=$id><button type = 'button' class= 'btn btn-sm btn-danger'>Excluir</button></a><td>";
